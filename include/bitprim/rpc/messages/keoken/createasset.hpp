...
bool createasset(nlohmann::json& json_object, int& error, std::string& error_code,
                        std::vector<libbitcoin::chain::input_point>& outputs_to_spend,  
                        libbitcoin::wallet::payment_address& asset_owner, uint64_t& utxo_satoshis, 
                        std::string& asset_name, uint64_t& asset_amount, bool use_testnet_rules, Blockchain& chain)
{
...
template <typename Blockchain>
nlohmann::json process_createasset(nlohmann::json const& json_in, Blockchain& chain, bool use_testnet_rules)
{
    ...
    if (createasset(result, error, error_code, outputs_to_spend, asset_owner, utxo_satoshis, asset_name, asset_amount, use_testnet_rules, chain))
    {
