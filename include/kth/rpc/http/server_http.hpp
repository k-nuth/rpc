...
namespace SimpleWeb {
    ...
    public:
        ...
        std::function<void(std::shared_ptr<typename ServerBase<socket_type>::Request>, boost::system::error_code const&)> on_error;
                    ...
                    while((param_end=line.find(':'))!=std::string::npos) {
                        ...
                        if ((value_start)<line.size()) {
                            ...
                            if (value_start<line.size()) {
                                ...
                                auto param = line.substr(0, param_end);
//                                auto value = line.substr(value_start, line.size() - value_start - 1);
                                auto value = line.substr(value_start, line.size() - value_start);

                                request->header.emplace(param, value);
                            }
                        }
        ...
        void write_response(const std::shared_ptr<socket_type> &socket, const std::shared_ptr<Request> &request,
            ...
            auto response=std::shared_ptr<Response>(new Response(socket), [this, request, timer](Response *response_ptr) {
                ...
                this->send(response, [this, response, request, timer](boost::system::error_code const& ec) {
                    if (timer)
                        timer->cancel();
                    if ( ! ec) {
                        float http_version;
    ...
    public:
        DEPRECATED Server(unsigned short port, size_t thread_pool_size=1, long timeout_request=60, long timeout_content=300) :
                Server() {
            config.port=port;
